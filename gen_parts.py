...
# ==============================================================================
# V3 renders every SKU page. It reuses the SAME SEO head, schema, breadcrumb,
# global header/footer shell and RFQ business logic as prior versions; only the
# SKU content area + layout differ (left hero + right sticky RFQ + sticky
# scroll-spy tabs). It renders ONLY real MASTER data — no fabricated/inferred
# specs, no price/stock/lead-time, no N/A. The legacy V2 renderer (gen_part_page)
# was removed on 2026-09-12; V3 is now the only render path.
...
# ---------------------------------------------------------------------------
# (2026-09-12) V3 is the SOLE SKU page renderer. V3_MPNS above is retained for
# reference only and no longer drives routing. All SKUs render via
# gen_part_page_v3. The legacy V2 renderer (gen_part_page) was removed; no
# legacy-exception allow-list remains.
# ---------------------------------------------------------------------------
...
def gen_component_category_page(cat_slug, cat_name, parts, all_rows=None, by_cat=None, noindex=None):
    ...
    # disk (rendered by gen_subcategory.py -- the v2.1 L3 generator). Deterministic + data-driven.
...
TEMPLATE_VERSION = "2026.09.v3.phase1"   # bump when gen_part_page_v3 body changes
...
def incremental_pipeline(args, groups, out_root, manifest_path=MANIFEST_PATH):
    ...
    node_created = set()
    try:
        # Fine(L3) + Category (top) pages -- CATEGORY SYSTEM, gated behind --regen-categories
        # (decoupling rule, 2026-09-16). Plain SKU incremental publish does NOT touch them.
        if args.regen_categories:
            for (_cslug, _fine, _l3slug) in sorted(affected_l3):
                ...
                os.makedirs(_d, exist_ok=True)  # L3 HTML via gen_subcategory.py
                node_written += 1
                if not _existed:
...
def main():
    ...
    ap.add_argument("--regen-categories", action="store_true",
                    help="CATEGORY PUBLISH ONLY: explicitly (re)generate Components / L1 / L2 / "
    ...
    # DECOUPLED (2026-09-16): Components / L1 / L2 / Subcategory pages belong to the CATEGORY
    # SYSTEM, not the SKU pipeline. They are generated ONLY on an explicit "category publish"
    # (--regen-categories). Plain SKU regeneration never touches them.
    if args.regen_categories:
        ...
        # L3 pages are rendered EXCLUSIVELY by gen_subcategory.py (v2.1 design: ItemList schema
        # + /page/2/ pagination + Manufacturers section). gen_parts.py only ensures the directory
        # exists so a new fine category is discoverable; HTML is produced in the delegated pass
